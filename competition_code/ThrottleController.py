...
def _env_sections(name):
    """Parse "2:3.45,6:3.4" into {2: 3.45, 6: 3.4}."""
    out = {}
...
# Tuning hooks for sweeping corner speed against a live server without editing
# this file between runs.  Both unset in competition, so they are a no-op there.
MU_SCALE = _env_float("ROAR_MU_SCALE", 1.0)
MU_SECTION = _env_sections("ROAR_MU_SECTION")
# 2026-09-15: 825 -> 815 — the earlier braking buys the L2 turn-in margin
# that removes the ~10%-per-pass l2_exit departure (the cost: +0.05 s; the
# brake-pulse latching quantizes it — KB 800 costs +0.55 s).  KB down was
# never tested by the original tuning (only 835/860/900 up — all DNF).
BRAKE_K = _env_float("ROAR_BRAKE_K", 815.0)

# Corner speed is exhausted - every section is a straight plus a single corner,
# so `mu` only acts over ~150 m and sections 2/6 DNF the moment it moves.  What
# has never been tuned is the *shape* of the braking: these four knobs decide
# how hard and how long the car slows once it has decided to, rather than how
# fast it may go.  All default to the values the 320.25 s run used.
LIGHT_BRAKE = _env_float("ROAR_LIGHT_BRAKE", 0.6)   # drag applied while at throttle
SPEED_CHG = _env_float("ROAR_SPEED_CHG", 2.4)       # kph/tick assumed for hysteresis
BRAKE_CAP = _env_float("ROAR_BRAKE_CAP", 8.0)       # max ticks of a latched hard brake
BRAKE_DIV = _env_float("ROAR_BRAKE_DIV", 3.0)       # kph of overspeed per latched tick
EARLY_THR = _env_float("ROAR_EARLY_THR", 2.5)       # "speed already moving fast" cutoff
...
class ThrottleController:
    ...
    def run(
        ...
        gear = max(1, int(current_speed / _env_float("ROAR_GEAR_DIV", 60.0)))
        if throttle < 0:
            gear = -1
    ...
    def speed_data_to_throttle_and_brake(self, speed_data: SpeedData):
        ...
        avg_speed_change_per_tick = SPEED_CHG  # Speed decrease in kph per tick
        ...
        if percent_of_max > 1:
            ...
            if percent_of_max > 1 + (
                ...
                if self.brake_ticks <= 0 and speed_change < EARLY_THR:
                    # start braking, and set for how many ticks to brake
                    self.brake_ticks = round((speed_data.current_speed - speed_data.recommended_speed_now) / BRAKE_DIV)
                    self.brake_ticks = min(int(BRAKE_CAP), self.brake_ticks)
                ...
                if percent_of_max > 1.02 or percent_speed_change > (
                    ...
                    return (1, LIGHT_BRAKE)  # light break, while keeping throttle on.
                else:
    ...
    def speed_for_turn(
        ...
        # BRAKE_K sets how late the car brakes: it is the effective
        # deceleration in this "how fast may I be, `distance` before a corner
        # that needs `target_speed`" curve.  Raising it brakes later.  Exposed
        # so it can be tuned against a live server; default 825 is unchanged.
        d = (1 / 675) * (target_speed**2) + distance
        max_speed = math.sqrt(BRAKE_K * d)
    ...
    def get_target_speed(self, radius: float, current_section: int):
        ...
        # Per-section grip, tuned section-by-section against a live CARLA
        # server.  Baseline (2.75 default; 3.35/3.3/2.85/3.3/2.1 for sections
        # 2/3/4/6/9) ran 321.25 s; these values run 320.25 s, repeatably and
        # with zero collisions.
        #
        # It had to be done one section at a time: a GLOBAL +2 % is already a
        # DNF, because sections 2 and 6 sit exactly on their limit and reject
        # any extra corner speed at all.  Section 4 gave the most (-0.50 s at
        # +10 %) and DNFs at +11 %.  Section 9's low 2.1 is deliberate - it
        # sets the car up for the start/finish straight, and moving it in
        # EITHER direction costs 0.2-1.0 s, so it is left alone.
        mu = 2.75
        ...
        if current_section == 1:
            # 2026-09-15: 2.97 -> 3.02 — the sec-1 widening (0.5 -> 0.8)
            # unpins the grip (the line-grip coupling); 3.03 crash-loops on
            # both the 0.5 and the 0.8 lines.
            mu = 3.02
        if current_section == 2:
            # Was 3.35 and genuinely pinned there - but only against the OLD
            # line.  Opening that corner from 54.5 m to 63.6 m moved the
            # constraint with the radius, exactly as sqrt(mu.g.R) predicts, and
            # 3.45 now runs clean where it used to DNF.  3.55 still DNFs.
            mu = 3.45
        if current_section == 3:
            mu = 3.36
        if current_section == 4:
            # 2026-09-15: 3.14 -> 3.24 — the sec-4 late-apex widening (the
            # corner R 43.1 was never widened) unpins the grip; 3.29 departs
            # at the L2 turn-in (the brake-target leak: the corner target
            # uses the car's CURRENT section mu, and the L2's braking zone
            # lies in this section).  Requires KB 815 (below).
            mu = 3.24     # DNFs at 3.17 on the un-widened line
        if current_section == 5:
            mu = 2.98
        if current_section == 6:
            mu = 3.3      # at its limit - do not raise
        if current_section == 7:
            mu = 2.85
        if current_section == 8:
            mu = 2.98
        if current_section == 9:
            mu = 2.1      # low on purpose; hurts if moved either way

        # Per-section grip trim, so corner speed can be tuned against a live
        # server without editing this file between runs.  MU_SCALE is a global
        # multiplier; MU_SECTION overrides individual sections, e.g.
        #   ROAR_MU_SCALE=1.02
        #   ROAR_MU_SECTION="2:3.45,6:3.4"
        # Both are unset in competition, so this is a no-op there.
        mu *= MU_SCALE
        if current_section in MU_SECTION:
            mu = MU_SECTION[current_section]
    ...
    def get_target_speed_new(self, radius: float, current_section: int, current_location):
        ...
        if current_section == 3:
            # the sec-3 second throttle model's grip constant.
            # 2026-09-15: 3.6 -> 3.65 — measured clean; 3.7 crashes at the
            # lesmo_exit on this line (the same lever Tate's 3rd-place
            # solution measured as a win on the Spring 2026 base).
            # Still env-tunable: ROAR_MU_S3_NEW (the default now 3.65).
            mu = _env_float("ROAR_MU_S3_NEW", 3.65)
